...
a = Analysis(
    ...
    datas=[
        ...
        *( [(str(SV / "u2netp.onnx"), ".")] if (SV / "u2netp.onnx").exists() else [] ),
        # SEED ATMOS depth generator — DATA files (run by the user's external torch
        # python via subprocess, never imported by the frozen app). Land in _internal/
        # where seed_atmos.REPO (= its own __file__.parent) resolves the default
        # SEED_ATMOS_ONESHOT path. Fixes "depth one-shot script missing" out of the box.
